...
{   
    public class SalesReturnService : ISalesReturn
    {
                ...
                using (var dataB = new Database9003Entities())
                {
                    ...
                    try
                    {
                        ...
                        for (int i = 0; i < oSalesReturn.Details.Count; i++)
                        {
                            ...
                            pt.brand_code = oSalesReturn.Details.ElementAt(i).BrandCode;
                            pt.size_code = oSalesReturn.Details.ElementAt(i).SizeCode;
                            pt.colour_code = oSalesReturn.Details.ElementAt(i).ColourCode;
                            pt.brand = oSalesReturn.Details.ElementAt(i).Brand;
                            pt.size = oSalesReturn.Details.ElementAt(i).Size;
                            pt.colour = oSalesReturn.Details.ElementAt(i).Colour;
                            //get a barcode here
            ...
            using (var dataB = new Database9003Entities())
            {
                ...
                if (cps.Count() > 0)
                {
                    ...
                    foreach (var item in cps)
                    {
                        ...
                        ccp.Details.Add(new CSalesReturnDetails() { SerialNo = (int)item.serial_no, ProductCode = item.product_code, Product = item.product, SalesReturnUnit = item.sales_unit, SalesReturnUnitCode = item.sales_unit_code, SalesReturnUnitValue = (decimal)item.sales_unit_value, Quantity = (decimal)item.quantity, SalesReturnRate = (decimal)item.sales_rate, Rate = (decimal)item.mrp, Total = (taxValue+netValue), Barcode = item.barcode, OldQuantity= (decimal)item.quantity, Tax=(decimal)item.tax, ProductDiscount=(decimal)item.product_discount, GrossValue=grossValue, TaxValue=taxValue, NetValue=netValue, BrandCode = item.brand_code, ColourCode = item.colour_code, SizeCode = item.size_code, Brand = item.brand, Size = item.size, Colour = item.colour });                        
                    }
                }
            ...
            using (var dataB = new Database9003Entities())
            {
                ...
                if (cps.Count() > 0)
                {
                    ...
                    foreach (var item in cps)
                    {
                        ...
                        decimal netValue = grossValue - (decimal)item.product_discount;
                        decimal taxValue = netValue * (decimal)item.tax / 100;
                        ccp.Details.Add(new CSalesReturnDetails() { SerialNo = serialNo++, ProductCode = item.product_code, Product = item.product, SalesReturnUnit = item.sales_unit, SalesReturnUnitCode = item.sales_unit_code, SalesReturnUnitValue = (decimal)item.sales_unit_value, Quantity = (decimal)item.quantity*-1, SalesReturnRate = (decimal)item.sales_rate, Rate = (decimal)item.mrp, Total = (taxValue+netValue), Barcode = item.barcode, OldQuantity = (decimal)item.quantity*-1, Tax=(decimal)item.tax, ProductDiscount=(decimal)item.product_discount, GrossValue=grossValue, TaxValue=taxValue, NetValue=netValue, BrandCode = item.brand_code, ColourCode = item.colour_code, SizeCode = item.size_code, Brand = item.brand, Size = item.size, Colour = item.colour });
                    }
                }
        ...
        //Reports
        public List<CSalesReturnReportDetailed> FindSalesReturnDetailed(DateTime startDate, DateTime endDate, string billType, string billNo, string customerCode, string customer, string productCode, string product, decimal tax, string brandCode, string brand, string colourCode, string colour, string sizeCode, string size, string narration, string financialCode)
        {
