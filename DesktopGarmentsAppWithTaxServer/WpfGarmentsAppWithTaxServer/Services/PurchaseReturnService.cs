                ...
                if (cps.Count() > 0)
                {
                    ...
                    foreach (var item in cps)
                    {
                        ...
                        if (quantity > 0)
                        {
                            ...
                            decimal netValue = grossValue - (decimal)item.product_discount;
                            decimal taxValue = netValue * (decimal)item.tax / 100;
                            ccp.Details.Add(new CPurchaseReturnDetails() { SerialNo = (int)serialNo++, ProductCode = item.product_code, Product = item.product, PurchaseReturnUnit = item.purchase_unit, PurchaseReturnUnitCode = item.purchase_unit_code, PurchaseReturnUnitValue = (decimal)item.purchase_unit_value, Quantity = (decimal)item.quantity, PurchaseReturnRate = (decimal)item.purchase_rate, MRP = (decimal)item.mrp, Total = (netValue + taxValue), Barcode = item.barcode, Tax = (decimal)item.tax, ProductDiscount = (decimal)item.product_discount, InterstateRate = (decimal)item.interstate_rate, WholesaleRate = (decimal)item.wholesale_rate, GrossValue = grossValue, NetValue = netValue, TaxValue = taxValue, OldQuantity=(decimal)item.quantity, BrandCode = item.brand_code, ColourCode = item.colour_code, SizeCode = item.size_code, Brand = item.brand, Size = item.size, Colour = item.colour });
                        }
                    }
                }
